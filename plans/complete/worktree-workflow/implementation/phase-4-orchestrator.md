# Phase 4: Worktree Workflow Orchestrator

> **For Claude:** REQUIRED SUB-SKILL: Use gremlins:executing-plans to implement this plan task-by-task.

**Goal:** Create the worktree-workflow orchestrator skill that chains all phases together with state tracking.

**Architecture:** Orchestrator skill that manages state, invokes sub-skills, and guides user through the development loop. State persists in `.claude/workflow-state.json`.

**Tech Stack:** Markdown skill file, JSON state file, bash for git operations.

**Related:** [../design/overview.md](../design/overview.md), [./phase-1-gate-skills.md](./phase-1-gate-skills.md)

---

## Task 1: Create worktree-workflow Skill

**Files:**
- Create: `skills/worktree-workflow/SKILL.md`

**Step 1: Create the skill directory**

```bash
mkdir -p skills/worktree-workflow
```

**Step 2: Create the SKILL.md file**

Create `skills/worktree-workflow/SKILL.md` with this content:
...
```bash
cat skills/worktree-workflow/SKILL.md | head -30
```
...
```bash
git add skills/worktree-workflow/SKILL.md
git commit -m "feat(skills): add worktree-workflow orchestrator

Adds orchestrator skill that chains the full development loop:
- State persistence in .claude/workflow-state.json
- Phase flow: review → implement → audit → reach/finish
- Invokes sub-skills for each phase
- Reach opportunities opt-in after passing audit
- Sync with base branch before finishing (rebase/merge options)
- Invokes plan-complete to move plan to complete/
...
```bash
ls -la skills/worktree-workflow/
grep "phase" skills/worktree-workflow/SKILL.md | head -10
git log --oneline -1
```

Expected:
- Directory exists with SKILL.md
- Phase references visible
- Commit for orchestrator skill
